...
def main(tags='2d',amount=50,offset=0):
    ...
    while len(imageSrcs)<amount:
        ...
        print('Getting links to the image posts from the search results')
        for i in range(len(h)):
            item=h[i]
            src=baseUrl+item.attrib['href']
            srcList.append(src)

        print('Getting the image sources from the posts in the search results')
        for i in srcList:
            done=False
            while done==False:
